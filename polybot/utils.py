...
def get_context(
    ...
    if isinstance(event, Message | ChatMemberUpdated):
        game = sm.storage.get_game(event.chat.id)

    elif isinstance(event, CallbackQuery):
        if event.message is None:
            game = sm.storage.get_player_game(event.from_user.id)
        else:
            game = sm.storage.get_game(event.message.chat.id)
    ...
    player = (
        ...
        else game.get_player(event.from_user.id)
    )
